...
class RecipeDetailViewTest(RecipeTestBase):
    def test_recipe_detail_view_function_is_correct(self):
        recipe_view = resolve(reverse('recipes:recipe_detail', kwargs={'pk':1}))
        self.assertEqual(recipe_view.func.view_class, views.RecipeDetailView)

    def test_recipe_detail_view_loads_correct_template(self):
        self.make_recipe()
        response = self.client.get(reverse('recipes:recipe_detail', kwargs={'pk':1}))
    ...
    def test_recipe_detail_view_returns_404_if_no_recipes_found_ok(self):
        response = self.client.get(reverse('recipes:recipe_detail', kwargs={'pk':10000}))
    ...
    def test_recipe_detail_dont_load_recipes_not_published(self):
        ...
        response = self.client.get(reverse('recipes:recipe_detail', kwargs={'pk':recipe.id}))
        self.assertEqual(response.status_code, 404)
